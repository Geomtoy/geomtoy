...
Promise.resolve("Let's begin")
    ...
    .then(async () => {
        ...
        let promises = _.range(seven).map(async index => {
            ...
            await Promise.all([sketchStrokeD.draw(G.Segment(data.centerPoint, data.heptagonPEighthPoints[index])), sketchStrokeD.draw(G.Segment(data.centerPoint, data.heptagonNEighthPoints[index]))]);
        });
